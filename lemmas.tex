...
\item Def: Let f be a mapping of a metric space X into MS Y. We say that f is $uniformly \; continuous$ on X if $\forall \epsilon > 0 \; \exists \delta > 0$ such that $\forall p,q \in X$:
...
\begin{itemize}
  ...
  If they are equal, we write $\mathboxinate{f \in \mathscr R(\alpha)}$.
...
%Anik's notes on Frenet
  ...
  $$f \in \mathscr R(\alpha) on [a,b] \iff \forall \epsilon > 0 \st \exists P \st
  ...
  $\implies \phi \circ f \in \mathscr R(\alpha)$
